...
trait WdlTest extends FlatSpec with Matchers {
  def resolver(root: File)(relPath: String): String = (root / relPath).contentAsString
  def loadWdlFile(wdlFile: File) = WdlNamespaceWithWorkflow.load(wdlFile.toJava, resolver(wdlFile / "..") _)
  def getTask(ns: WdlNamespaceWithWorkflow, name: String): Task = ns.tasks.find(_.unqualifiedName == name).get
